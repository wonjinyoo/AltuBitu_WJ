...
// 앞,뒤 모두에서 삭제하고 삽입이 가능하다는 점에서 덱을 이용한 것임!

using namespace std;

// 덱에 있는 문자하나하나를 문자열로 이어붙이는 함수
string dqToString(deque<char>& dq) {
    // 덱의 모든 문자를 이어붙인 결과를 담을 변수, 아무것도 없는 문자열로 초기화!
    string ans = "";
    while (!dq.empty()) {
        // ans 변수에 dq의 문자를 하나하나 pop하면서 이어붙이기
        ans += dq.front();
        dq.pop_front();
    }
    return ans;
...
string findPass(string s) {
    ...
    // 커서를 기준으로 왼쪽, 오른쪽으로 넣는다는 아이디어가 포인트..!
    // 커서 기준 왼쪽의 문자열을 pass에 담고
    string pass = dqToString(dq_left);
    // 그 문자열에 커서 기준 오른쪽 문자열을 이어붙인다
