...
// 과자의 길이가 len일 때, 몇 명에게 나누어줄 수 있는지
int childNum(int len, vector<int>& snacks) {
    ...
    for (int i = snacks.size() - 1; i >= 0; i--) { //가장 긴 과자부터 검사
        ...
        // 한 과자당 몇명에게 나누어줄 수 있는지 cnt에 더해줌
        cnt += (snacks[i] / len);
    }

    // 모든 과자들에 대해 몇 명에게 나누어줄 수 있는지 더해진 값이 리턴
    return cnt;
}
...
int upperSearch(int left, int right, int target, vector<int>& snacks) {
    while (left <= right) {
        ...
        // 더 적은 아이들에게 나눠줄 수 있으면 => 과자길이를 줄여야
        else {
            right = mid - 1;
        }
    }
